...
int ** bareiss(int ** now,int ** next,int row,int col,int pivIndex,int prevPiv){
    ...
    int i=0,j=0,pivot=now[pivIndex][pivIndex],temp,flag;

  /*  printf("pivIndex %d,pivot %d \n",pivIndex,pivot);  */
    flag=0;
    if(pivot==0){
      for(i=0;i<col;i++){
        if(now[pivIndex][i]!=0) flag=1;
      }
      if(flag){
        for(i=0;i<col;i++){
          temp=now[pivIndex][i];
          now[pivIndex][i]=now[pivIndex+1][i];
          now[pivIndex+1][i]=temp;
        }
      pivot=now[pivIndex][pivIndex];
    /*  printf("PIVOT %d\n",pivot);  */
      }
      else return now;
      /* Ortada 000 olmasi durumu */
    }
...
/*
  for(i=0;i<col;i++){
    for(j=0;j<2;j++) printf("%d ",katsayi[i][j]);
    printf("\n");
  }
