...
public:
    ...
    Car& operator=(const Car& rhs); // assignment overloaded
    Laptime * getHead() const;
    void setHead(Laptime* newHead);
    Laptime * getTail() const;
    void setTail(Laptime* newTail);
    void setPerformance();
    void setNext(Car * newNext);
    Car * getNext() const;
    void setDriverName(std::string name);
    void clearLaptimeLL();
    void pushBackLaptime(const int& laptime);
    void popBackLaptime();
