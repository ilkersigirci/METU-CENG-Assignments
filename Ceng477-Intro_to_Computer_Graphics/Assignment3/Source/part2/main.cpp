...
GLint verticalPieces = 125;
GLint horizontalPieces = 250;
int verticesNumber = 6 * verticalPieces * horizontalPieces;
...
void sendTextureData()  {
	...
	glUniform1f(glGetUniformLocation(idProgramShader, "pi"), pi);
}
...
static void keyCallback(GLFWwindow *win, int key, int scancode, int action, int mods) { // GLFWwindow *window default
	...
	switch(key) {
		...
		case GLFW_KEY_Q:{ // offset left
			cam.offset += float(1) / verticalPieces;
			glUniform1f(glGetUniformLocation(idProgramShader, "offset"  ), cam.offset);
			break;
		}
...
int main(int argc, char * argv[]) {
	...
	lightPosition = glm::vec3(0,1600,0);
